...
function MaterialRow({
  ...
  item: MaterialListData;
  onEdit: (item: MaterialListData) => void;
}) {
  return (
    <View style={s.row}>
      <View style={s.rowLeft}>
      ...
      <Text style={s.rate}>{parseFloat(item.rate).toFixed(2)}</Text>
...
export default function MaterialListScreen() {
    ...
    async (deptId: string, pageNum: number, reset = false) => {
      ...
      try {
        ...
        const res = await fetch(
          `${ApiEndPoints.MATERIAL_LIST}?dept_id=${deptId}&page=${pageNum}&limit=${PAGE_LIMIT}`,
          {
            headers: {
              Authorization: `Bearer ${token}`,
              "Content-Type": "application/json",
        ...
        if (json.success === 1 && Array.isArray(json.data)) {
          ...
          setMaterials((prev) => (reset ? json.data : [...prev, ...json.data]));
  ...
  useEffect(() => {
    ...
    setPage(1);
    setMaterials([]);
    fetchMaterials(selectedDept.dept_id, 1, true);
  }, [selectedDept]);
  ...
  const onEndReached = useCallback(() => {
    ...
    setPage(nextPage);
    setLoadingMore(true);
    fetchMaterials(selectedDept.dept_id, nextPage);
  }, [hasMore, loadingMore, selectedDept, page, fetchMaterials]);
  ...
  const onEditMaterial = useCallback((item: MaterialListData) => {
    router.push({
      pathname: "/(main)/store_management/material/AddEditMaterialScreen",
      ...
      <View style={s.tableContainer}>
        {loading ? (
          ...
          <FlatList
            ...
            renderItem={({ item }) => (
              <MaterialRow item={item} onEdit={onEditMaterial} />
            )}
