...
export default function OTPVerificationScreen() {
  ...
  const handleVerifyOTP = async () => {
    if (!otp) {
      AppUtils.showToast('Please enter the OTP');
      return;
    }
    ...
    try {
      ...
      if (res.success) {
        AppUtils.showToast(res.data.message || 'OTP verified successfully');
        router.push({
          pathname: '/ResetPasswordScreen',
          params: { email }
        });
      } else {
        AppUtils.showToast(res.data?.message || 'Invalid OTP');
      }
    } catch (error) {
        ...
        <View style={styles.content}>
          <View style={styles.headerBlock}>
            <View style={styles.iconBox}>
              <Ionicons name="keypad-outline" size={32} color={T.white} />
            </View>
            <Text style={styles.heading}>Verify OTP</Text>
            <Text style={styles.subheading}>
              We've sent a 4-digit code to {email}. Please enter it below to continue.
            </Text>
          </View>
...
const styles = StyleSheet.create({
  ...
  resendText: { color: T.textMuted, fontSize: 14, fontWeight: '600' },
});
