    ...
    <TouchableOpacity
      ...
      onPress={() =>
        router.push({
          pathname:
            type === "Sales"
              ? "/sales/SaleDetailScreen"
              : "/purchase/PurchaseDetailScreen",
          params:
            type === "Sales"
              ? { saleId: item.saleId }
              : { purchaseId: item.saleId },
        })
      }
    >
      {/* Left */}
      <View style={s.rowLeft}>
        <View style={s.rowTop}>
          ...
          <View
            style={[
              s.badge,
              { backgroundColor: stt.bg, borderColor: stt.border },
            ]}
          >
        ...
        <View style={s.rowMeta}>
          <Text style={[s.voucherNo, { color: amountColor }]}>
            {item.voucherNo}
          </Text>
          ...
          >
            ...
            {counts[tab.key] > 0 && (
              <View style={[s.tabBadge, isActive && s.tabBadgeActive]}>
                <Text
                  style={[s.tabBadgeText, isActive && s.tabBadgeTextActive]}
                >
                  {counts[tab.key]}
                </Text>
              </View>
...
export default function SalesInvoiceScreen() {
  ...
  const filteredEntries = useMemo(() => {
    ...
    if (paymentFilter !== "all") {
      ...
      if (chip)
        result = result.filter((e) => chip.match.includes(e.paymentStatus));
    }
    // text search
    if (searchQuery) {
  ...
  const totalOutstanding = useMemo(
    () =>
      entries.reduce(
        (sum, r) => sum + Number.parseFloat(r.outstanding || "0"),
        0,
      ),
    [entries],
  );
  ...
  const onRefresh = useCallback(async () => {
    ...
    setRefreshing(true);
    await Promise.all([
      runSync(month, year, "Sales"),
      runSync(month, year, "Purchase"),
    ]);
    setRefreshing(false);
  ...
  const handleFilterApply = useCallback(
    (m: number, y: number) => {
      ...
      DeviceEventEmitter.emit("SHARED_FILTER_CHANGED", {
        month: m,
        year: y,
        fy: selectedFY,
      });
      ...
      <View style={s.viewSummary}>
        <View style={s.viewSummaryLeft}>
          ...
          <Text style={s.viewSummarySub}>
            {activeTab} ·{" "}
            {chips.find((c) => c.key === paymentFilter)?.label ?? "All"} ·{" "}
            {MONTH_SHORT[month]} {year}
          </Text>
        </View>
        <View style={s.viewSummaryRight}>
          <Text style={s.viewSummaryAmount}>{fmt(totalOutstanding)}</Text>
          <Text style={s.viewSummaryCount}>
          ...
          headerRight: () => (
            ...
            >
              ...
              >
                <Ionicons
                  name={
                    isSearchVisible ? "close-circle-outline" : "search-outline"
                  }
                  size={24}
...
const s = StyleSheet.create({
  ...
  listContent: { paddingHorizontal: 8, paddingTop: 4, paddingBottom: 80 },
