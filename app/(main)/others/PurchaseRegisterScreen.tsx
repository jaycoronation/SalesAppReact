...
// Dot color per bucket for the filter chips
...
function statusStyle(statusDisplay: string, isOverdue: string) {
    ...
    if (statusDisplay?.toLowerCase() === 'paid') return { bg: '#D1FAE5', text: '#065F46', border: '#6EE7B7' }
    return { bg: '#FEF9C3', text: '#92400E', border: '#FDE68A' }
}

// ─── Register Row ─────────────────────────────────────────────────────────────

function RegisterRow({ item, accent }: { item: PurchaseRegisterEntry; accent: string }) {
...
export default function PurchaseRegisterScreen() {
    ...
    // ── Active bucket state (drives both data and UI) ──────────────────────────
    ...
    const loadLocal = useCallback(async () => {
        const records = await loadPurchaseRegister(bucket)
        setEntries(records)
    }, [bucket])

    const runSync = useCallback(async () => {
        await syncPurchaseRegister(bucket, fy)
        await loadLocal()
    }, [bucket, fy, loadLocal])

    // Reload whenever bucket changes
    useEffect(() => {
        setEntries([])
        loadLocal()
        setSyncing(true)
    ...
    if (entries.length === 0 && syncing) {
        return (
            <View style={s.center}>
                <Stack.Screen options={{ title: bucketLabel, headerBackTitle: '', headerShown: true, }} />
    ...
    return (
        <View style={s.container}>
            <Stack.Screen options={{ title: `Payables · ${bucketLabel}`, headerBackTitle: '', headerShown: true, }} />
            ...
            <View style={[s.heroCard, { backgroundColor: bucketCfg.bg, borderColor: bucketCfg.border }]}>
                <View style={s.heroTopRow}>
                    ...
                    <View style={s.heroTopRight}>
                        ...
                        <TouchableOpacity
                            ...
                            activeOpacity={0.7}
                        >
                            <Text style={[s.filterBtnText, { color: bucketCfg.accent }]}>
                                ⇅ Filter
                            </Text>
                        </TouchableOpacity>
                    </View>
                ...
                <Text style={s.heroSub}>Outstanding across {totalRecords} bills</Text>
            ...
            <FlatList
                data={entries}
                keyExtractor={(item) => `${item.btwnDays}-${item.purchaseId}-${Math.random()}`}
                renderItem={({ item }) => (
                    <RegisterRow item={item} accent={bucketCfg.accent} />
                )}
                ...
                ListEmptyComponent={
                    <View style={s.empty}>
                        <Text style={s.emptyText}>No records for this bucket</Text>
                    </View>
                }
            ...
            >
                <BottomSheetView style={s.sheetContent}>
                    ...
                    <Text style={s.sheetSubtitle}>Select a bucket to view payables</Text>
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <View style={s.bucketOptionMid}>
                                        <Text style={[s.bucketOptionLabel, isActive && { color: cfg.text, fontWeight: '700' }]}>
                                            {BUCKET_LABELS[b]}
                                        </Text>
                                        <Text style={s.bucketOptionDesc}>
                                            {b === 'paid'
                                                ? 'Fully settled bills'
                                                : b === 'd0_7' ? 'Due within a week'
                                                    : b === 'd7_15' ? 'Due in 7–15 days'
                                                        : b === 'd15_30' ? 'Due in 15–30 days'
                                                            : 'Overdue by 30+ days'}
                                        </Text>
                                    </View>
...
const s = StyleSheet.create({
    ...
    heroSub: { fontSize: 12, color: '#6B7280', marginBottom: 14 },
    ...
    sheetClose: { fontSize: 16, color: '#9CA3AF', fontWeight: '400' },
